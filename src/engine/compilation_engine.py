...
class CompilationEngine:
    ...
    def build_package_command(
        self, file_path, tool, output_path=None, extra_args=None, target="native"
    ):
        ...
        if self.config and language:
            ...
            if config_cmd:
                log.info(
                    f"[CompilationEngine] Usando comando desde .compilador para {language.get('language')}: {config_cmd}"
                )
    ...
    def build_command_for(
        ...
        if output_type == "wasm":
            if shutil.which("emcc"):
                ...
                cmd = [
                    ...
                    file_path,
                    "-o",
                    output_path or (os.path.splitext(file_path)[0] + ".wasm"),
                ]
                return cmd, None, []
        ...
        if strategy:
            ...
            return strategy.build_command(
                file_path, output_path, extra_args, output_type, release_mode, target
            )
        ...
        cmd, cwd, post_actions = self._legacy_fallback(
    ...
    def package(self, file_path, tool, output_path=None, extra_args=None, target="native"):
        ...
        cmd, cwd, post_actions = self.build_package_command(
            file_path, tool, output_path, extra_args, target
        )
        if not cmd:
            return {
                "success": False,
