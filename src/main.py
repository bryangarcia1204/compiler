...
class CompileWorker(QThread):
    ...
    def __init__(
        self, engine, file_path, tool, output_path, extra_args, output_type, release_mode=False
    ):
        super().__init__()
        self.engine = engine
    ...
    def run(self):
        try:
                ...
                self.finished.emit(
                    {
                        "success": False,
                        "stdout": "",
                        "stderr": "No se pudo construir el comando.",
                        "returncode": -1,
                        "output_file": None,
                    }
                )
            ...
            t_out = threading.Thread(
                target=read_stream,
                args=(self._process.stdout, self.line_stdout, self._stdout_buffer),
                daemon=True,
            )
            t_err = threading.Thread(
                target=read_stream,
                args=(self._process.stderr, self.line_stderr, self._stderr_buffer),
                daemon=True,
            )
            t_out.start()
            t_err.start()
            ...
            result = {
                ...
                "output_file": (
                    output_file
                    if (self.tool.get("type") == "compiler" or self.action == "package")
                    else None
                ),
            }
            show = {
            ...
            log.debug(
                f"[Main]Datos de salida de la compilacion: {result.get('stdout'), result.get('stderr')}"
            )
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.project_gen_btn.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_FileDialogNewFolder", 0))
        )
        ...
        self.refresh_tools_btn.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_BrowserReload", 0))
        )
        ...
        self.market_btn.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_FileDialogDetailedView", 0))
        )
        ...
        self.output_path_edit.setPlaceholderText(
            "Ruta de salida (opcional, dejar vacío para automático)"
        )
        self.output_browse_btn = QPushButton("Examinar...")
        self.output_browse_btn.setIcon(
            self.style().standardIcon(getattr(QStyle, "SP_DirOpenIcon", 0))
        )
        ...
        self.status_label.setStyleSheet(
            "background-color: #3a3a3a; padding: 6px; border-radius: 4px; font-weight: bold;"
        )
    ...
    def open_project_generator(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo cargar el generador de proyectos:\n{e}"
            )
    ...
    def show_argument_suggestions(self):
        ...
        if not args_list:
            QMessageBox.information(
                self, "Sin sugerencias", f"No hay argumentos predefinidos para {tool_name}."
            )
            return
        ...
        if last_file and os.path.exists(last_file):
            ...
            if lang_info:
                self.lang_info_label.setText(
                    f"Lenguaje: {lang_info['language']} | Tipo: {lang_info['type'].capitalize()}"
                )
                ...
                if last_tool and self.available_tools:
                    for i, t in enumerate(self.current_tools):
                        if t.get("name") == last_tool.get("name") and t.get(
                            "command"
    ...
    def update_tools_for_language(self, lang_info):
        ...
        if not filtered:
            ...
            if suggestions:
                msg = (
                    f"No se encontraron herramientas para {lang_info['language']}. Sugerencias de instalación:\n"
                    + "\n".join(f"  • {s}" for s in suggestions)
                )
                self.append_log(msg, kind="warning")
            else:
                self.append_log(
                    f"No se encontraron herramientas para {lang_info['language']}.", kind="warning"
                )
        else:
            ...
            if lang_type == "interpreter":
                ...
                if not has_packager:
                    ...
                    if suggestions:
                        msg = (
                            "Tienes el intérprete instalado, pero no hay empaquetador. Para generar ejecutables instala:\n"
                            + "\n".join(f"  • {s}" for s in suggestions)
                        )
                        self.append_log(msg, kind="warning")
                else:
                    self.append_log(
                        "✅ Herramienta de empaquetado detectada. Puedes empaquetar este script seleccionando el modo 'Empaquetar'.",
                        kind="info",
                    )
        ...
        if file_path:
            ...
            if lang_info:
                ...
                QMessageBox.warning(
                    self,
                    "Advertencia",
                    "No se pudo determinar el lenguaje. Algunas herramientas pueden no funcionar.",
                )
    ...
    def update_mode_buttons(self):
        if self.current_file:
            ...
            has_packager = any(
                t["type"] == "packager" and ext in t.get("extensions", [])
    ...
    def start_action(self):
        ...
        if self.mode_package.isChecked() and self.selected_tool.get("type") == "packager":
            ...
            self.append_log(
                "Se iniciará el proceso. Revisa la consola para más detalles.", kind="info"
            )
        else:
            action = "compile"
            self.logs_text.clear()
    ...
    def save_current_config(self, last_file=None):
        try:
            ...
            if self.selected_tool:
                cfg["last_tool"] = {
                    "name": self.selected_tool.get("name"),
                    "command": self.selected_tool.get("command"),
                }
