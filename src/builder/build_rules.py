...
class BuildRules:
    ...
    def build_order(cls, languages: List[str], summary: dict[str, Any] | None = None) -> List[str]:
        ...
        graph = {
            r.name: [
                dep.name
                for dep in cls._rules.values()
                if dep.produces and any(a in r.requires for a in dep.produces)
            ]
            for r in relevant
        }
...
def _register_default_rules(tools: list):
    ...
    BuildRules.register(
        BuildRule(
            ...
            produces=[
                BuildArtifact.OBJECT_FILE,
                BuildArtifact.STATIC_LIB,
                BuildArtifact.SHARED_LIB,
            ],
            requires=[],
    ...
    BuildRules.register(
        BuildRule(
            ...
            requires=[
                BuildArtifact.PYD_MODULE,
                BuildArtifact.SO_MODULE,
            ],  # Depende de C++ si existe
            input_extensions=[".py"],
