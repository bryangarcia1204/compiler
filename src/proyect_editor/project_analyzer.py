...
BUILD_ARCHITECTURES = {
    ...
    "java_maven": {
        ...
        "build_cmd": "mvn clean compile package",
        "outputs": ["jar"],
    },
    "java_gradle": {
    ...
    "dotnet_publish": {
        "config_files": ["*.csproj", "*.sln"],
        "build_cmd": "dotnet publish -c Release",
        "outputs": ["exe"],
    },
    "node_npm": {
...
class ProjectAnalyzer:
    ...
    def analyze(self, archivo=None) -> Dict[str, Any]:
        ...
        log.info(
            f"[ProjectAnalyzer] Análisis completado. Tipo: {self.summary['project_type']}, "
    ...
    def _update_compilador(self):
        ...
        self.compilador.set(
            "suggested_config_files", self.summary.get("suggested_config_files", [])
        )
        self.compilador.set(
            "suggested_build_architecture", self.summary.get("suggested_build_architecture", "")
        )
        ...
        if build_plan:
            ...
            if not targets:
                targets = [
                    {"name": "default", "description": "Compilación por defecto", "steps": []}
                ]
    ...
    def _is_binary_file(self, file_path: str) -> bool:
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                printable = all(
                    c < 128 and (c == 9 or c == 10 or c == 13 or 32 <= c <= 126) for c in chunk
                )
    ...
    def _analyze_cpp_content(self, entry: Dict):
        ...
        if "class" in content or "struct" in content:
            if "pybind11" not in content:
                self.summary["evidence"].append(
                    f"Archivo {entry['rel_path']} contiene definiciones de clase/struct"
                )
    ...
    def _classify_project_type(self):
        ...
        if self.summary["languages"]:
            ...
            if main_lang == "python":
                ...
                has_cpp = any(
                    e["language"] in ("c", "cpp", "c++") for e in self.summary["source_files"]
                )
                ...
                if any(
                    dep in self.summary["dependencies"] for dep in ["click", "argparse", "typer"]
                ):
                    scores["cli"] += 2

                # Detectar web
                if any(
                    dep in self.summary["dependencies"]
                    for dep in ["flask", "django", "fastapi", "aiohttp"]
                ):
                    scores["web"] += 2
                ...
                if any(
                    dep in self.summary["dependencies"]
                    for dep in ["tkinter", "PyQt5", "wxPython", "PySide"]
                ):
                    scores["gui"] += 2
                ...
                if (
                    "tensorflow" in self.summary["dependencies"]
                    or "torch" in self.summary["dependencies"]
                ):
                    scores["data_science"] += 2
            ...
            # Rust
                ...
                if any(
                    "cdylib" in f["content"]
                    for f in self.summary["config_files"]
            ...
            # JavaScript/TypeScript
                ...
                if "package.json" in [e["name"] for e in self.summary["config_files"]]:
                    ...
                    if any(
                        dep in self.summary["dependencies"]
                        for dep in ["react", "vue", "angular", "express"]
                    ):
                        scores["web"] += 2
                    if any(
                        dep in self.summary["dependencies"]
                        for dep in ["commander", "yargs", "chalk"]
                    ):
                        scores["cli"] += 2
        ...
        if scores:
            ...
            self.summary["score_breakdown"] = dict(
    ...
    def _prepare_summary_for_ai(
        self, include_content: bool = False, max_content_size: int = 2000
    ) -> Dict:
        """
        ...
        try:
            ...
            response = self.ai_client.chat(
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un analista de proyectos experto en compilación. Responde SOLO con el JSON.",
                    },
            ...
            if data.get("confidence", 0) > self.summary.get("intent_confidence", 0):
                self.summary["project_type"] = data.get(
                    "project_type", self.summary["project_type"]
                )
                self.summary["intent_confidence"] = data.get(
                    "confidence", self.summary["intent_confidence"]
                )
            ...
            log.info(
                f"[ProjectAnalyzer] Sugerencias de IA aplicadas: {data.get('project_type')} (confianza: {data.get('confidence', 0)})"
            )

        except json.JSONDecodeError as e:
            log.warning(f"[ProjectAnalyzer] No se pudo parsear JSON de IA: {e}")
            log.debug(
                f"[ProjectAnalyzer] Respuesta que falló: {response[:300] if response else 'None'}"
            )
        ...
        try:
            ...
            response = self.ai_client.chat(
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un analista de proyectos que vas a dar tu "
            ...
            if "project_type" in veredict:
                log.info(
                    f"[ProjectAnalyzer] Veredicto obtenido: {veredict.get('project_type')} (confianza: {veredict.get('intent_confidence', 0)})"
                )
                return veredict
            else:
                ...
                if "child_process" in content or "spawn" in content or "exec" in content:
                    ...
                    for py_file in matches:
                        dependencies["js_python_childprocess"].append(
                            {"js_file": entry["rel_path"], "python_script": py_file}
                        )
