...
class GenerateWorker(QThread):
    ...
    def run(self):
        try:
            result = self.generator.generate_config_files(
                self.project_info, self.custom_prompt, self.selected_targets
            )
    ...
    def run(self):
        try:
            result = self.generator.enhance_files_with_ai(
                self.project_info, self.existing_files, self.custom_prompt
            )
            self.finished.emit(result)
        except Exception as e:
            self.error.emit(str(e))
...
class ProjectGeneratorDialog(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.provider_combo.addItems(
            ["Plataformia", "DeepSeek", "OpenAI", "Groq", "HuggingFace", "Qwen-Code"]
        )
        ...
        self.prompt_edit.setPlaceholderText(
            "Describe qué tipo de proyecto quieres generar o qué mejoras quieres..."
        )
        ...
        if state:
            ...
            if project_dir and os.path.isdir(project_dir):
                ...
                if reply == QMessageBox.Yes:
                    ...
                    QTimer.singleShot(
                        100,
                        lambda: self.info_label.setText(
                            self.analyzer.get_summary() if self.analyzer else "Proyecto cargado"
                        ),
                    )
    ...
    def analyze_project(self):
        ...
        if self.ai_checkbox.isChecked() and self.analyzer.ai_client:
            ...
            if veredict:
                ...
                self.compilador.set(
                    "project.type", veredict.get("project_type", self.project_info["project_type"])
                )
                self.compilador.save()
            else:
                self.info_label.setText(
                    "⚠️ No se pudo obtener veredicto de IA. Usando análisis estándar."
                )
    ...
    def enhance_files_with_ai(self):
        ...
        self.enhance_worker = EnhanceWorker(
            self.generator, self.project_info, current_files, prompt
        )
    ...
    def _detect_output_type(
        self, project_type: str, language: str, binary_target: str, main_file: str
    ) -> str:
    ...
    def _detect_build_command(
        ...
        # ── Rust ──
            ...
            if "Cargo.toml" in [
                e.get("name", "") if isinstance(e, dict) else e for e in config_files
            ]:
                return {
                    "cmd": ["cargo", "build"],
                    "cwd": project_dir,
        ...
        try:
            if len(cmd) > 1 and "&&" in cmd:
                ...
                result = subprocess.run(
                    cmd_str, shell=True, cwd=cwd, capture_output=True, text=True, timeout=timeout
                )
            else:
                result = subprocess.run(
    ...
    def _compile_with_engine(self, main_file: str, output_type: str) -> Dict:
        ...
        output_file = os.path.join(
            output_dir, os.path.splitext(os.path.basename(main_file))[0] + ext
        )
